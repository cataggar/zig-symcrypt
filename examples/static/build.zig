...
pub fn build(b: *std.Build) void {
    ...
    const libraries = b.option(
        ...
        "symcrypt_libraries",
        "Ordered exact SymCrypt static library files",
    ) orelse &.{};
    ...
    const executable = b.addExecutable(.{ .name = "static-consumer", .root_module = module });
    const run = b.addRunArtifact(executable);
    b.step("run", "Run the static package consumer").dependOn(&run.step);
}
